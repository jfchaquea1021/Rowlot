...
(function() {
            ...
            return RowlotService.getUsers().then(function(response) {
                ...
                for (var i in response) {
                    ...
                    if(isNaN(response[i].data.Medalla)){
                        response[i].data.Medalla = 0;
                    }
                    if(isNaN(response[i].data.Vida)){
                        response[i].data.Vida = 0;
                    }
                    ...
                    if (typeof response[i].data.task != "undefined") {
                        ...
                        if ($scope.showStudent) {
                            angular.forEach(response[i].data.task, function(value, key) {
                                angular.forEach(value.students, function(v, k) {
    ...
    function TaskController($scope, $timeout, RowlotService, CurrentUserService, toastr, $stateParams, $state) {
        ...
        $scope.func = {
                    ...
                    .then(function(response) {
                        if (angular.isDefined(response.task)) {
                            angular.forEach(response.task, function(value, key) {
                                if ($scope.idTask == key) {
                                    $scope.task = value;
                                    $scope.dataFileStudent = $scope.func.getFileStudent();
                                }
                            });
                        } else {
                            $state.go('auth.rowlot-listtask');
            ...
            getFileStudent: function() {
                var arrFileStudent = [];
                angular.forEach($scope.task.students, function(value, key) {
                    if ($scope.showStudent) {
                        if (firebase.auth().currentUser.uid == key) {
                            value.uid = key;
                            value.profile = Object(value.profile);
                ...
                console.log(arrFileStudent);
                return arrFileStudent;
            },
            sendFile: function() {
                ...
                uploadTask.on('state_changed', function(snapshot) {
                    ...
                    RowlotService.putTask($scope.idUnidad, $scope.idTask, $scope.profile.uid, data)
                        .then(function(data) {
                            $state.go('auth.rowlot-listtask');
            ...
            showModalFile: function(fileSelected, $index) {
                $scope.fileSelected = {};
                $scope.fileSelected.porcentaje = (fileSelected.porcentaje) ? fileSelected.porcentaje : "";
                $scope.fileSelected.calificacion = (fileSelected.calificacion) ? fileSelected.calificacion : "";
                $scope.idUserSelected = fileSelected.uid;
                $scope.indexSelected = $index;
                $("#fileSelected").modal('show');
            },
    ...
    LibraryController.$inject = ['$scope', '$timeout', 'RowlotService', "CurrentUserService", "toastr", '$stateParams', '$state'];

    function LibraryController($scope, $timeout, RowlotService, CurrentUserService, toastr, $stateParams, $state) {
        ...
        $scope.func = {
            init: function() {
                RowlotService.getCurrentUser().then(function(response) {
                    ...
                    $scope.func.getLibrary();
                }, function(error) {
                    console.log(error);
                });
            },
            getLibrary: function() {
                RowlotService.getLecturas()
                    .then(function(data) {
                        $scope.lecturas = data;
                    })
                    .catch(function(err) {
            ...
            addLectura: function() {
                var element = document.getElementById("fileLectura");
                if (element.files.length < 1) {
                    toastr.error('La lectura debe tener archivo adjunto.', 'Archivo de lectura');
                    return;
                }

                var storageRef = firebase.storage().ref();
                var uploadTask = storageRef.child('lectura/' + element.files[0].name).put(element.files[0]);
                ...
                uploadTask.on('state_changed', function(snapshot) {
                    ...
                    var archivoAdjunto = uploadTask.snapshot.downloadURL;
                    var imagenAdjunto = "http://saveabandonedbabies.org/wp-content/uploads/2015/08/default.png";
                    var elementImage = document.getElementById("portadaLectura");

                    if (elementImage.files.length) {
                        var imgPortada = storageRef.child('lectura/' + elementImage.files[0].name).put(elementImage.files[0]);
                        imgPortada.on('state_changed', function(snapshot) {
                            console.log(snapshot);
                        }, function(error) {
                            console.error(error);
                        }, function() {
                            imagenAdjunto = imgPortada.snapshot.downloadURL;
                            var data = { Titulo: $scope.newLectura.Titulo, Descripcion: $scope.newLectura.Descripcion, Url: archivoAdjunto, ImagenUrl: imagenAdjunto };
                            $scope.func.setLectura(data);
                        });
                    } else {
                        var data = { Titulo: $scope.newLectura.Titulo, Descripcion: $scope.newLectura.Descripcion, Url: archivoAdjunto, ImagenUrl: imagenAdjunto };
                        $scope.func.setLectura(data);
            ...
            setLectura: function(data) {
                RowlotService.addLectura(data)
                    .then(function(response) {
